...
void inline TFT_iter32_p2(sfixn *A,sfixn *W){
  ...
  */
  ...
  if(n<=basecase){//N=17,basecase=32
	...
	//}
      ...
      DFT_eff_p2(bigern, e, invectmp, KRT, SB1);
	...
	DFT_eff_p2(bigern, e, invec, KRT, SB1);
	
	Shuffle_tft(bigern,invec,SB1);
      }
      else{

	TFT_Basecase_p2(bigern, e, invec, KRT, SB1);
	  ...
	  DFT_eff_p2(n, e, invec, KRT, SB1);
	
	  Shuffle_tft(n,invec,SB1);
      }
      else{

	TFT_Basecase_p2(n, e, invec, KRT, SB1);
 ...
 ///////right part and twddile part////////////////////////////////////////
    ...
    */
	...
	for(int j=0;j<new_r_b;j++){
	  ...
	  DFT_eff_p2(new_r_n, right_step, invectmp+pi, KRT+f, SB1);//KRT+n is for level 2 omega
	...
	cilk_for(int j=0;j<new_r_b;j++){
	  ...
	  if(new_r_n>32){ 
	    ...
	    TFT_Basecase_p2(new_r_n, right_step, invectmp+pi, KRT+f, SB1);//KRT+n is for level 2 omega
	  }
  ...
  */
	...
	*/
	...
	if(new_l_n>32){
	  DFT_eff_p2(new_l_n, left_step, invec+pii, KRT+d,SB1+pii);//KRT+n is for level 2 omega
	  Shuffle_tft(new_l_n, invec+pii,SB1+pii);
	}
	else{

	  TFT_Basecase_p2(new_l_n, left_step, invec+pii, KRT+d,SB1+pii);//KRT+n is for level 2 omega
	}
	...
	*/
	...
	TFT_Core_p2(oldn, new_l_n, new_l_l, new_l_m, basecase, relax_size,KRT,e,SB1+i*new_l_n,invec+i*new_l_n,invectmp+i*new_l_n);
  ...
  void ITFT_DFT_p2(int L2,int step_L2,sfixn *KRT,int d_L2,sfixn *SB, sfixn *SB2){
    ...
    DFT_eff_p2(L2, step_L2, SB, KRT+d_L2, SB2);
  ...
  void ITFT_Core_p2(int oldn, int L, sfixn zeta,sfixn onedivzeta, int z, int n, int f, sfixn *KRT,sfixn *invKRT, sfixn *invec, sfixn *SB2){
	...
	ITFT_DFT_p2(L2,step_L2,KRT,d_L2,SB,SB2);
	...
	for(int i=1;i<L2;i++){//for each row, there are L2 items	  
      ...
      for(int u=0;u<n1;u++){//u is indexing for row
	ITFT_Core_p2(oldn,L2,new_zeta1,onedivzeta,L2,L2,0,KRT,invKRT,invec+u*L2,SB);
	//for(int i=0;i<L2;i++){//for each row, there are L2 items
	//invec[u*L2+i]=SB[i];
	//}
      }
	...
	//std::cout<<" rightmost 1 input invec "<< u+i*L2 <<" is "  << invec[u+i*L2] <<std::endl;
      ...
      ITFT_Core_p2(oldn,L1,new_zeta2,new_onedivzeta,z1+1,n1,f_quote,KRT,invKRT,SB,SB);//not sure for omega
	...
	//std::cout<<" rightmost 2 SB_"<<i<<" is "<<SB[i]<<std::endl;
      ...
      ITFT_Core_p2(oldn,L1,new_zeta2,new_onedivzeta,z1,n1,f_quote,KRT,invKRT,SB,SB);
	...
	//std::cout<<" last row SB_  [n1*L2+i]  is "<<n1*L2+i <<std::endl;
      ...
      */
      ITFT_Core_p2(oldn,L2,new_zeta1,onedivzeta,z2_quote,n2,f,KRT,invKRT,invec+n1*L2,SB);
	...
	//std::cout<<" leftmost 1 SB_"<<i<<" is "<<SB[i]<<std::endl;
      ...
      ITFT_Core_p2(oldn,L1,new_zeta2,onedivzeta,z1+1,n1+1,0,KRT,invKRT,SB,SB);//not sure for omega
      for(int i=0;i<L1;i++){// for each col, there are L1 items
	invec[u+i*L2]=SB[i];
	...
	//std::cout<<"left most 2 SB_"<<i<<" is "<<SB[i]<<std::endl;
      ...
      sfixn new_omega = MontMulModSpe_OPT3_AS_GENE_INLINE(1,KRT[d+u]);
      sfixn new_zeta2 = MulMod(new_omega,zeta,p);
      ITFT_Core_p2(oldn,L1,new_zeta2,onedivzeta,z1,n1+1,0,KRT,invKRT,SB,SB);//not sure for omega
      for(int i=0;i<L1;i++){// for each col, there are L1 items
	invec[u+i*L2]=SB[i];
  ...
  void ITFT_Wrapper_p2(int oldn, int L, sfixn zeta,sfixn onedivzeta, int z, int n, int f, sfixn *KRT,sfixn *invKRT, sfixn *invec, sfixn *SB,int basecase, sfixn *invectmp, int relax_size){
    ...
    /*
  ITFT_Core_p2(oldn,L,zeta,onedivzeta,z,n,f,KRT,invKRT,invec,SB2);
  for (int i = 0; i < oldn; ++i){
    ...
    //sfixn trynum = MontMulModSpe_OPT3_AS_GENE_INLINE(RINV2,2701251764010509990);
  ...
  //ITFT_Core_p2(oldn,L,zeta,onedivzeta,z,n,f,KRT,invKRT,invec,SB); //compute in normal mode 
  Mont_ITFT_Core_p2(oldn,L,0,z,n,f,KRT,invKRT,invec,mont_inv2,SB,p);//compute in montgemory mode
  ...
  */
  ...
  void Mont_ITFT_Core_p2(int oldn, int L, int index_zeta, int z, int n, int f, sfixn *KRT,sfixn *invKRT, sfixn *invec,  sfixn mont_inv2, sfixn *SB, sfixn p){
	...
	ITFT_DFT_p2(L2,step_L2,KRT,d_L2,SB+u*L2,SB3+u*L2);
	...
	//Mont_ITFT_Core_p2(oldn,L2,new_index_zeta1,L2,L2,0,KRT,invKRT,invec+u*L2,mont_inv2,SB);//for serial
	Mont_ITFT_Core_p2(oldn,L2,new_index_zeta1,L2,L2,0,KRT,invKRT,invec+u*L2,mont_inv2,SB+u*L2,p);
      }
	...
	Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1+1,n1,f_quote,KRT,invKRT,invec+u*L1,mont_inv2,SB+u*L1,p);
	...
	int index_new_zeta2 = ( temp_zeta2 * u + index_zeta) % oldn;      
	Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1,n1,f_quote,KRT,invKRT,invec+u*L1,mont_inv2,SB+u*L1,p);
	...
	temp_row[i]=invec[n1+i*L1];
      ...
      Mont_ITFT_Core_p2(oldn,L2,new_index_zeta1,z2_quote,n2,f,KRT,invKRT,temp_row,mont_inv2,SB,p);
	...
	Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1+1,n1+1,0,KRT,invKRT,invec+u*L1,mont_inv2,SB+u*L1,p);//not sure for omega
      ...
      Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1,n1+1,0,KRT,invKRT,invec+u*L1,mont_inv2,SB+u*L1,p);//not sure for omega
    ...
    cilk_for(int u=n2;u<m_quote;u++){// u is indexing for column		   
      ...
      int index_new_zeta2 = ( temp_zeta2 * u + index_zeta) % oldn;
      //Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1+1,n1,f_quote,KRT,invKRT,SB+u*L1,mont_inv2,invec);//for serial
      Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1+1,n1,f_quote,KRT,invKRT,SB+u*L1,mont_inv2,invec+u*L1,p);
    }
    ...
    cilk_for(int u=m_quote;u<z2_quote;u++){// u is indexing for column
      ...
      int index_new_zeta2 = ( temp_zeta2 * u + index_zeta) % oldn;
      //Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1,n1,f_quote,KRT,invKRT,SB+u*L1,mont_inv2,invec);//for serial
      Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1,n1,f_quote,KRT,invKRT,SB+u*L1,mont_inv2,invec+u*L1,p);
    }
	...
	temp_row[i]=SB[n1+i*L1];
      ...
      Mont_ITFT_Core_p2(oldn,L2,new_index_zeta1,z2_quote,n2,f,KRT,invKRT,temp_row,mont_inv2,invec,p);
    ...
    cilk_for(int u=0;u<m;u++){
      ...
      //Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1+1,n1+1,0,KRT,invKRT,SB+u*L1,mont_inv2,invec);//not sure for omega //for serial
      Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1+1,n1+1,0,KRT,invKRT,SB+u*L1,mont_inv2,invec+u*L1,p);//not sure for omega 
    }
    ...
    cilk_for(int u=m;u<n2;u++){
      ...
      //Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1,n1+1,0,KRT,invKRT,SB+u*L1,mont_inv2,invec);//not sure for omega//for serial
      Mont_ITFT_Core_p2(oldn,L1,index_new_zeta2,z1,n1+1,0,KRT,invKRT,SB+u*L1,mont_inv2,invec+u*L1,p);//not sure for omega
      }
