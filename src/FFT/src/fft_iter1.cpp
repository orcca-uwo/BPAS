...
void  unrolledSpe8AddSubSSEMod(sfixn* a,sfixn* b){
	sfixn prime[2] = {MY_PRIME1,MY_PRIME1};
	asm("movdqa (%0),%%xmm1\n\t"
		"movdqa (%1),%%xmm2\n\t"
		"movdqa 16(%0),%%xmm5\n\t"
		"movdqa 16(%1),%%xmm6\n\t"
...
//	q2=(usfixn)q2>>(BASE-NPOW1);
//	q2*=SEE1;
//	a += (a >> BASE_1) & MY_PRIME1;
//	a+=q2;
//	if(a<MY_PRIME1)
//		return a;
//	else
...
static  sfixn MontMulModSpeDEBUG_OPT3_AS_GENE(sfixn a, sfixn b){
	...
	std::cout<<a << " "<<b<<std::endl;
	q2 = b*INV_PRIME1;
	std::cout<<q2 << " "<<INV_PRIME1<<std::endl;
	MulAndAddHiLoUnsigned(&a, &b,q2,MY_PRIME1);
	std::cout<<a << " "<<b<<std::endl;
	a -= MY_PRIME1;
	std::cout<<a << " "<<b<<std::endl;
	a += (a >> BASE_1) & MY_PRIME1;
...
sfixn testMontMul(usfixn u,usfixn v){
	usfixn w = MulMod(u,v>>RSFT1,MY_PRIME1);
	w = MulMod(w,RINV1,MY_PRIME1);
	return w;
}
...
void DFT_rec_p1(int n, int r,sfixn *A,sfixn *W,sfixn *B){
	if (n==1) {    
		return;
	}
		...
		//Shuffle(n, A, B);
		sfixn *W2 = W+n; 
		DFT_rec_p1(n2, r1, A, W2, B);
		DFT_rec_p1(n2, r1, A+n2, W2, B);
