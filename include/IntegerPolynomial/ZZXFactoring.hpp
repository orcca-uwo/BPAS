...
extern void print_NTL_mat_ZZ(NTL::mat_ZZ& M);
extern void print_vec_long(const NTL::vec_long& a);
...
inline void copyLocalInfoT(LocalInfoT* mlit, LocalInfoT& lit) {

	mlit->n = lit.n;
	mlit->NumPrimes = lit.NumPrimes;
	mlit->NumFactors = lit.NumFactors;
	mlit->p = lit.p;
	mlit->pattern = lit.pattern;
	mlit->PossibleDegrees = lit.PossibleDegrees;

}

inline void set_factor_data(factor_data_t& fd, vec_ZZX factors, ZZX f, vec_ZZX w, ZZ P, long p, long e, LocalInfoT& LocalInfo, long bnd) {
	...
	fd.e = e;
	fd.bnd = bnd;
	copyLocalInfoT(&fd.LocalInfo,LocalInfo);

}



inline void mul(ZZ_pX& x, vec_ZZ_pX& a)
...
inline void mul(ZZX& x, const vec_pair_ZZX_long& a)
{
   long l = a.length();
   ZZX res;
   long i, j;
...
//				cerr << W[kk] << std::endl;
            RemoveFactors(W, I);
//			cerr << "after RemoveFactors(W,I):" << std::endl;
//			for (int kk=0; kk<W.length(); ++kk)
//				cerr << W[kk] << std::endl;
            r = W.length();
            cnt = 0;
...
void FindTrueFactors(vec_ZZX& factors, const ZZX& ff, 
                     ...
                     long bnd)
{
   std::cerr << "non vH FindTrueFactors called:" << std::endl;
   ZZ_pBak bak;
   bak.save();
   ZZ_p::init(P);
...
const long van_hoeij_card_thresh = 1;
...
//   std::cerr << "d = " << d << ", root_bound = " << root_bound << ", n = " << n << std::endl;
   ...
   while (t2 <= t1) {
//      std::cerr << "i = " << i << ", t1 = " << t1 << ", t2 = " << t2 << std::endl;
      i++;
      t2 *= p;
   }
             ...
             long verbose)
{
   ...
   ZZ half_P;
   cerr << "P = " << ZZ_p::modulus() << std::endl;
   RightShift(half_P, ZZ_p::modulus(), 1);
   cerr << "half_P = " << half_P << std::endl;
                    ...
                    long n, long ran_bits)
{
   ...
   if (root_bound == 1) {
      ...
      std::cerr << "if: t1 = " << t1 << std::endl;
   }
   else {
      t1 = (power(root_bound, d)*n) << (ran_bits + 2);
      std::cerr << "else: t1 = " << t1 << std::endl;
   }
...
void FindTrueFactors_vH(vec_ZZX& factors, const ZZX& ff, 
                        ...
                        long bnd)
{
   std::cerr << "vH FindTrueFactors called:" << std::endl;
...
// coefficient
{
   ...
   if (verbose) {
      cerr << "*** start SFFactor in ZZXFactoring.hpp.\n";
   }
...
//   cerr << "f = " << f << std::endl;
//   cerr << "LeadCoeff(f) = " << LeadCoeff(f) << std::endl;
...
//   cerr << "lift_bnd = " << lift_bnd << std::endl;
...
//      cerr << "t1 = " << t1 << std::endl;
      if (sign(P) < 0)
         LogicError("whoops!!!");
      InvMod(t1, t1, P);
//      cerr << "t1 = " << t1 << ", P = " << P << std::endl;
      f1.rep.SetLength(n+1);
      for (i = 0; i <= n; i++) {
//         cerr << "f.rep[" << i << "] = " << f.rep[i] << ", t1 = " << t1 << std::endl;
         mul(t2, f.rep[i], t1);
//         cerr << "t2 = " << t2 << ", P = " << P << std::endl;
         rem(f1.rep[i], t2, P);
      }
   }
   cerr << "f1 = " << f1 << std::endl;
...

{
   ...
   if (!ZZXFac_PowerHack) {
      ok_to_abandon = 0;
      ll_SFFactor(factors, ff, verbose, bnd, fd);
      return;
   }
   ...
   while (!done) {
      ...
      for (j = 0; j < res.length(); j++) {
         ...
         ll_SFFactor(res2, res[j], verbose, k < 0 ? bnd : 0, fd);
...

{
   ...
   content(c, ff);
   divide(ff, ff, c);
   cerr << "after content divide: ff = " << ff << std::endl;
   ...
   for (i = 0; i < sfd.length(); i++) {
      ...
      cerr << "input to SFFactor: sfd[i].a = " << sfd[i].a << std::endl;
      SFFactor(x, sfd[i].a, verbose, bnd, fd);
...
// START OF REPEATED ROUTINES //

inline void _gauss(NTL::ZZ& d_out, NTL::mat_ZZ& R_out, const NTL::mat_ZZ& M) {

   long n = M.NumRows();
   long m = M.NumCols();

   if (n == 0 || m == 0) NTL::LogicError("gauss: internal error");
...
inline void _CutAway(NTL::mat_ZZ& B1, NTL::vec_ZZ& D, NTL::mat_ZZ& M, long C, long r, long d) {

   long k = M.NumRows();
   NTL::ZZ bnd = 4*NTL::to_ZZ(C)*NTL::to_ZZ(C)*NTL::to_ZZ(r) + NTL::to_ZZ(d)*NTL::to_ZZ(r)*NTL::to_ZZ(r);
...
inline void _BuildReductionMatrix(NTL::mat_ZZ& M, long& C, long r, long d, const NTL::ZZ& pdelta, const NTL::vec_vec_ZZ& chop_vec, const NTL::mat_ZZ& B_L, long verbose) {
...
inline void _ChopTraces(NTL::vec_ZZ& C, const NTL::vec_ZZ& Tr, long d, const NTL::vec_ZZ& pb, const NTL::ZZ& pdelta, const NTL::ZZ& P, const NTL::ZZ& lc) {

   if (d <= 0) NTL::LogicError("ChopTraces: internal error (1)");
   if (C.length() < d) NTL::LogicError("ChopTraces: internal error (2)");
   if (Tr.length() < d) NTL::LogicError("ChopTraces: internal error (3)");
   if (pb.length() < d) NTL::LogicError("ChopTraces: internal error (4)");
   if (P <= 1) NTL::LogicError("ChopTraces: internal error (5)");
...
inline void Compute_Trace(NTL::vec_ZZ& Tr, const NTL::ZZX& f, long d, const NTL::ZZ& P) {
   ...
   if (d <= 0)
      NTL::LogicError("ComputeTrace: internal error (2)");

   if (Tr.length() < d)
      NTL::LogicError("ComputeTrace: internal error (3)");

   if (P <= 1)
      NTL::LogicError("ComputeTrace: internal error (4)");
...
inline void _Compute_pdelta(long& delta, NTL::ZZ& pdelta, long p, long bit_delta)
{
   NTL::ZZ t1;
   long i;
...
inline void _Compute_pb(NTL::vec_long& b, NTL::vec_ZZ& pb, long p, long d, const NTL::ZZ& root_bound, long n)
{
   NTL::ZZ t1, t2;
   long i;
...
inline void _Compute_pb_eff(long& b_eff, NTL::ZZ& pb_eff, long p, long d, const NTL::ZZ& root_bound, long n, long ran_bits) {
   ...
   if (root_bound == 1) {
      t1 = (NTL::to_ZZ(d)*NTL::to_ZZ(n)) << (ran_bits + 1);
      std::cerr << "if: t1 = " << t1 << std::endl;
   }
   else {
      t1 = (power(root_bound, d)*n) << (ran_bits + 2);
...
inline void _FindTrueFactors_vH(NTL::vec_ZZX& factors, const NTL::ZZX& ff, const NTL::vec_ZZX& w, const NTL::ZZ& P, long p, long e, NTL::LocalInfoT& LocalInfo, long verbose, long bnd)
{
   const long SkipSparse = 0;
   ...
   while (2*k <= W.length() && 
      ...
      if (k <= 1)
         ...
         CardinalitySearch1(factors, f, W, LocalInfo, k, bnd, verbose);
      k++;
      std::cerr << "end while loop: k <= van_hoeij_card_thresh = " << (k <= van_hoeij_card_thresh) << ", W.length() <= van_hoeij_size_thresh = " << (W.length() <= van_hoeij_size_thresh) << std::endl;
   }
...
/*             ((d >= 2 && s > 128) || (d >= 3 && s > 32) || (d >= 4 && s > 8) ||*/
/*              d >= 5) ) {*/
/*            if (verbose) std::cerr << "   abandoning\n";*/
/*            append(factors, f);*/
/*            break;*/
/*         }*/
